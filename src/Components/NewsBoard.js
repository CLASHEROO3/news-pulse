    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const clean = allArticles.filter(a => 
          ...
          a.title !== "[Removed]" && 
          a.description && 
          a.description.length > 5
        );

        // Sort and Unique
        const unique = Array.from(new Set(clean.map(a => a.url))).map(url => clean.find(a => a.url === url));
    ...
    <div>
      {loading ? <div className="spinner-center" style={{margin:'100px auto', width:'40px', height:'40px', border:'4px solid #ddd', borderTopColor:'#c59235', borderRadius:'50%', animation:'spin 1s linear infinite'}}></div> : (
        <div className="news-container">
          {articles.map((news, i) => (
            <NewsItem key={i} index={i} title={news.title} description={news.description} urlToImage={news.urlToImage} url={news.url} sourceName={news.source.name} />
          ))}
        </div>
      )}
    </div>
