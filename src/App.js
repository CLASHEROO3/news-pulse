    ...
    <div className="App">
      ...
      <div className={`sidebar-drawer ${isMenuOpen ? 'open' : ''}`}>
        ...
        <div className="sidebar-content">
          <button className={`side-link ${activeView === 'bookmarks' ? 'active' : ''}`} onClick={() => {setActiveView('bookmarks'); setIsMenuOpen(false);}}>⭐ Saved Articles</button>
          <hr />
      ...
      <div className="sub-nav">
        <button 
          className={`sub-nav-btn ${activeView === 'for-you' ? 'active' : ''}`} 
        ...
        {CATEGORIES.map(cat => (
          <button 
            key={cat} 
            className={`sub-nav-btn ${activeView === cat ? 'active' : ''}`} 
      ...
      <footer className="footer-final">
        <p>© {new Date().getFullYear()} NewsPulse Aggregator | Built by [Your Names]</p>
      </footer>
    </div>
  );
}
